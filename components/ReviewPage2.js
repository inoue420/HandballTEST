...
/*     const interval = setInterval(() => {
  ...
  const handleAnswerButtonClick = async () => {
    ...
    const correctAnswers = currentQuestion ? currentQuestion.correctAnswers : [];
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        <View style={styles.contentContainer}>
          ...
          <View style={styles.optionsContainer}>
            {questionIdList.length > 0 && (
              questions.find(question => question.id === questionIdList[questionIndex]).options.map((option) => (
                <ChoiceButton
                  key={option}
                  label={`${String.fromCharCode(97 + questions.find(question => question.id === questionIdList[questionIndex]).options.indexOf(option))}) ${option}`}
